...
public final class CredentialStoreIndex {
    ...
    public void observe(EventRecord event) {
        evidence.observe("credential", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("credential") || attr.getKey().contains("vault")) {
