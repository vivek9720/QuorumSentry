...
public final class UserDirectory {
    ...
    public void put(String key, String department, String risk) {
    ...
    public void observe(EventRecord event) {
        evidence.observe("user", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("user") || attr.getKey().contains("department")) {
