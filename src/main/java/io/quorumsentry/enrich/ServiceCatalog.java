...
public final class ServiceCatalog {
    ...
    public void put(String key, String tier, String exposure) {
    ...
    public void observe(EventRecord event) {
        evidence.observe("service", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("service") || attr.getKey().contains("tier")) {
