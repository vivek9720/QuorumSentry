...
public final class CloudAccountIndex {
    ...
    public void observe(EventRecord event) {
        evidence.observe("account", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("account") || attr.getKey().contains("provider")) {
