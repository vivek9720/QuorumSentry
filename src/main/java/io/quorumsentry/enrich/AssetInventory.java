...
public final class AssetInventory {
    ...
    public void put(String key, String owner, String criticality) {
    ...
    public void observe(EventRecord event) {
        evidence.observe("asset", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("asset") || attr.getKey().contains("owner")) {
