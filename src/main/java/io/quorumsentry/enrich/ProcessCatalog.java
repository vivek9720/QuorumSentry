...
public final class ProcessCatalog {
    ...
    public void put(String key, String publisher, String rarity) {
    ...
    public void observe(EventRecord event) {
        evidence.observe("process", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("process") || attr.getKey().contains("publisher")) {
