...
public final class DevicePostureIndex {
    ...
    public void observe(EventRecord event) {
        evidence.observe("device", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("device") || attr.getKey().contains("posture")) {
