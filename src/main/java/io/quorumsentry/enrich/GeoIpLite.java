...
public final class GeoIpLite {
    ...
    public void observe(EventRecord event) {
        evidence.observe("ip", event.host(), event.timestamp(), event.severity().weight(), 3);
        for (Map.Entry<String, String> attr : event.attributes().entrySet()) {
            if (attr.getKey().contains("ip") || attr.getKey().contains("country")) {
