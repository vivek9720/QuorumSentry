...
public final class DataExfiltrationDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "exfil")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Large transfer with exfiltration language observed.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("exfil") || flow.tags().containsKey("exfil") || flow.protocol().contains("exfil");
        boolean volumeMatch = flow.bytes() > 1000000 && "bytes".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("bytes".equals("port") || "exfil".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Large transfer with exfiltration language observed.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("exfil") || indicator.type().contains("exfil") || indicator.value().contains("exfil");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Large transfer with exfiltration language observed.");
