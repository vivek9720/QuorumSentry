...
public final class RareProcessDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "process")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Rare executable path appeared in host telemetry.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("process") || flow.tags().containsKey("process") || flow.protocol().contains("process");
        boolean volumeMatch = flow.bytes() > 1000000 && "exe".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("exe".equals("port") || "process".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Rare executable path appeared in host telemetry.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("process") || indicator.type().contains("process") || indicator.value().contains("process");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Rare executable path appeared in host telemetry.");
