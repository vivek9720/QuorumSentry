...
public final class AdminProtocolDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "rdp")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Administrative protocol used in a suspicious context.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("rdp") || flow.tags().containsKey("rdp") || flow.protocol().contains("rdp");
        boolean volumeMatch = flow.bytes() > 1000000 && "port".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("port".equals("port") || "rdp".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Administrative protocol used in a suspicious context.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("rdp") || indicator.type().contains("rdp") || indicator.value().contains("rdp");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Administrative protocol used in a suspicious context.");
