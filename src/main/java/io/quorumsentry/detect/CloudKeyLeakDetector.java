...
public final class CloudKeyLeakDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "AKIA")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Cloud access key material appeared in logs.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("AKIA") || flow.tags().containsKey("AKIA") || flow.protocol().contains("AKIA");
        boolean volumeMatch = flow.bytes() > 1000000 && "key".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("key".equals("port") || "AKIA".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Cloud access key material appeared in logs.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("AKIA") || indicator.type().contains("AKIA") || indicator.value().contains("AKIA");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Cloud access key material appeared in logs.");
