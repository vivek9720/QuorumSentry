...
public final class ImpossibleTravelDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "geo")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "User geography changed too quickly for normal travel.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("geo") || flow.tags().containsKey("geo") || flow.protocol().contains("geo");
        boolean volumeMatch = flow.bytes() > 1000000 && "user".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("user".equals("port") || "geo".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "User geography changed too quickly for normal travel.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("geo") || indicator.type().contains("geo") || indicator.value().contains("geo");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "User geography changed too quickly for normal travel.");
