...
public final class PacketStormDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "packets")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Packet count exceeded local storm threshold.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("packets") || flow.tags().containsKey("packets") || flow.protocol().contains("packets");
        boolean volumeMatch = flow.bytes() > 1000000 && "flow".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("flow".equals("port") || "packets".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Packet count exceeded local storm threshold.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("packets") || indicator.type().contains("packets") || indicator.value().contains("packets");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Packet count exceeded local storm threshold.");
