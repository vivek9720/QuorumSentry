...
public final class BackupTamperDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "backup")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Backup tamper wording appeared in telemetry.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("backup") || flow.tags().containsKey("backup") || flow.protocol().contains("backup");
        boolean volumeMatch = flow.bytes() > 1000000 && "host".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("host".equals("port") || "backup".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Backup tamper wording appeared in telemetry.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("backup") || indicator.type().contains("backup") || indicator.value().contains("backup");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Backup tamper wording appeared in telemetry.");
