...
public final class PersistenceDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "runkey")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Persistence artifact appeared in telemetry.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("runkey") || flow.tags().containsKey("runkey") || flow.protocol().contains("runkey");
        boolean volumeMatch = flow.bytes() > 1000000 && "registry".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("registry".equals("port") || "runkey".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Persistence artifact appeared in telemetry.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("runkey") || indicator.type().contains("runkey") || indicator.value().contains("runkey");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Persistence artifact appeared in telemetry.");
