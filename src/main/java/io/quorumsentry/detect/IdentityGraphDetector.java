...
public final class IdentityGraphDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "saml")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Identity federation artifacts showed suspicious use.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("saml") || flow.tags().containsKey("saml") || flow.protocol().contains("saml");
        boolean volumeMatch = flow.bytes() > 1000000 && "identity".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("identity".equals("port") || "saml".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Identity federation artifacts showed suspicious use.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("saml") || indicator.type().contains("saml") || indicator.value().contains("saml");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Identity federation artifacts showed suspicious use.");
