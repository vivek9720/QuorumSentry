...
public final class LateralMovementDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "psexec")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Remote execution tooling appeared in event text.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("psexec") || flow.tags().containsKey("psexec") || flow.protocol().contains("psexec");
        boolean volumeMatch = flow.bytes() > 1000000 && "host".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("host".equals("port") || "psexec".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Remote execution tooling appeared in event text.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("psexec") || indicator.type().contains("psexec") || indicator.value().contains("psexec");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Remote execution tooling appeared in event text.");
