...
public final class SuspiciousArchiveDetector extends AbstractDetector {
    ...
    @Override
    public String name() {
        return "SuspiciousArchiveDetector";
    }
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "archive")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Archive handling activity has suspicious indicators.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("archive") || flow.tags().containsKey("archive") || flow.protocol().contains("archive");
        boolean volumeMatch = flow.bytes() > 1000000 && "file".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("file".equals("port") || "archive".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Archive handling activity has suspicious indicators.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("archive") || indicator.type().contains("archive") || indicator.value().contains("archive");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Archive handling activity has suspicious indicators.");
