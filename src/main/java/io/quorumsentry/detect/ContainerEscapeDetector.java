...
public final class ContainerEscapeDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "cgroup")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Container boundary markers appeared with escape language.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("cgroup") || flow.tags().containsKey("cgroup") || flow.protocol().contains("cgroup");
        boolean volumeMatch = flow.bytes() > 1000000 && "container".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("container".equals("port") || "cgroup".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Container boundary markers appeared with escape language.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("cgroup") || indicator.type().contains("cgroup") || indicator.value().contains("cgroup");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Container boundary markers appeared with escape language.");
