...
public final class PayloadEntropyDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "entropy")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "High entropy token may be packed payload or secret.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("entropy") || flow.tags().containsKey("entropy") || flow.protocol().contains("entropy");
        boolean volumeMatch = flow.bytes() > 1000000 && "payload".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("payload".equals("port") || "entropy".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "High entropy token may be packed payload or secret.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("entropy") || indicator.type().contains("entropy") || indicator.value().contains("entropy");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "High entropy token may be packed payload or secret.");
