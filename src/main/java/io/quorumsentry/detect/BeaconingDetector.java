...
public final class BeaconingDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "beacon")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Periodic low-volume traffic can indicate command-and-control.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("beacon") || flow.tags().containsKey("beacon") || flow.protocol().contains("beacon");
        boolean volumeMatch = flow.bytes() > 1000000 && "interval".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("interval".equals("port") || "beacon".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Periodic low-volume traffic can indicate command-and-control.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("beacon") || indicator.type().contains("beacon") || indicator.value().contains("beacon");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Periodic low-volume traffic can indicate command-and-control.");
