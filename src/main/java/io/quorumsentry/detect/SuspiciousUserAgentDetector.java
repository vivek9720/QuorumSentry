...
public final class SuspiciousUserAgentDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "user_agent")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Unusual user-agent indicator appeared in telemetry.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("user_agent") || flow.tags().containsKey("user_agent") || flow.protocol().contains("user_agent");
        boolean volumeMatch = flow.bytes() > 1000000 && "http".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("http".equals("port") || "user_agent".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Unusual user-agent indicator appeared in telemetry.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("user_agent") || indicator.type().contains("user_agent") || indicator.value().contains("user_agent");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Unusual user-agent indicator appeared in telemetry.");
