...
public final class ServiceDiscoveryDetector extends AbstractDetector {
    ...
    @Override
    public String name() {
        return "ServiceDiscoveryDetector";
    }
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "scan")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Low-packet privileged flow suggests service discovery.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("scan") || flow.tags().containsKey("scan") || flow.protocol().contains("scan");
        boolean volumeMatch = flow.bytes() > 1000000 && "port".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("port".equals("port") || "scan".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Low-packet privileged flow suggests service discovery.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("scan") || indicator.type().contains("scan") || indicator.value().contains("scan");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Low-packet privileged flow suggests service discovery.");
