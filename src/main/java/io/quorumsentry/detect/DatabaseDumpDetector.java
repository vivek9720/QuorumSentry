...
public final class DatabaseDumpDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "dump")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Database dump behavior appeared in network or event telemetry.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("dump") || flow.tags().containsKey("dump") || flow.protocol().contains("dump");
        boolean volumeMatch = flow.bytes() > 1000000 && "database".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("database".equals("port") || "dump".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Database dump behavior appeared in network or event telemetry.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("dump") || indicator.type().contains("dump") || indicator.value().contains("dump");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Database dump behavior appeared in network or event telemetry.");
