...
public final class RansomwarePrepDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "shadowcopy")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Backup deletion behavior resembles ransomware preparation.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("shadowcopy") || flow.tags().containsKey("shadowcopy") || flow.protocol().contains("shadowcopy");
        boolean volumeMatch = flow.bytes() > 1000000 && "host".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("host".equals("port") || "shadowcopy".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Backup deletion behavior resembles ransomware preparation.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("shadowcopy") || indicator.type().contains("shadowcopy") || indicator.value().contains("shadowcopy");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Backup deletion behavior resembles ransomware preparation.");
