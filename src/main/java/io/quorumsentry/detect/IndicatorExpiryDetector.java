...
public final class IndicatorExpiryDetector extends AbstractDetector {
    ...
    @Override
    public String name() {
        return "IndicatorExpiryDetector";
    }
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "expired")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.LOW, event.host(), "Threat indicator contains an unusual validity window.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("expired") || flow.tags().containsKey("expired") || flow.protocol().contains("expired");
        boolean volumeMatch = flow.bytes() > 1000000 && "indicator".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("indicator".equals("port") || "expired".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.LOW, flow.sourceIp(), "Threat indicator contains an unusual validity window.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("expired") || indicator.type().contains("expired") || indicator.value().contains("expired");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.LOW, indicator.id(), "Threat indicator contains an unusual validity window.");
