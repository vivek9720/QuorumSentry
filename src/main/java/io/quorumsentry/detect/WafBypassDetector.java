...
public final class WafBypassDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "waf")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Web application firewall bypass wording appeared.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("waf") || flow.tags().containsKey("waf") || flow.protocol().contains("waf");
        boolean volumeMatch = flow.bytes() > 1000000 && "http".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("http".equals("port") || "waf".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Web application firewall bypass wording appeared.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("waf") || indicator.type().contains("waf") || indicator.value().contains("waf");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Web application firewall bypass wording appeared.");
