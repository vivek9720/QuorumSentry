...
public final class HoneytokenDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "honey")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, event.host(), "Honeytoken string was touched by an actor.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("honey") || flow.tags().containsKey("honey") || flow.protocol().contains("honey");
        boolean volumeMatch = flow.bytes() > 1000000 && "token".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("token".equals("port") || "honey".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, flow.sourceIp(), "Honeytoken string was touched by an actor.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("honey") || indicator.type().contains("honey") || indicator.value().contains("honey");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.CRITICAL, indicator.id(), "Honeytoken string was touched by an actor.");
