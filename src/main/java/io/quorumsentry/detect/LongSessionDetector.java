...
public final class LongSessionDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "duration")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.LOW, event.host(), "Long-lived session merits analyst review.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("duration") || flow.tags().containsKey("duration") || flow.protocol().contains("duration");
        boolean volumeMatch = flow.bytes() > 1000000 && "flow".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("flow".equals("port") || "duration".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.LOW, flow.sourceIp(), "Long-lived session merits analyst review.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("duration") || indicator.type().contains("duration") || indicator.value().contains("duration");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.LOW, indicator.id(), "Long-lived session merits analyst review.");
