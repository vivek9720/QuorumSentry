...
public final class PolicyBypassDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "bypass")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, event.host(), "Policy bypass language appeared in rule or event data.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("bypass") || flow.tags().containsKey("bypass") || flow.protocol().contains("bypass");
        boolean volumeMatch = flow.bytes() > 1000000 && "rule".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("rule".equals("port") || "bypass".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, flow.sourceIp(), "Policy bypass language appeared in rule or event data.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("bypass") || indicator.type().contains("bypass") || indicator.value().contains("bypass");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.HIGH, indicator.id(), "Policy bypass language appeared in rule or event data.");
