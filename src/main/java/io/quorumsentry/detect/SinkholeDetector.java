...
public final class SinkholeDetector extends AbstractDetector {
    ...
    public void inspectEvent(EventRecord event, DetectionContext context) {
        ...
        if (contains(event, "sinkhole")) {
            eventMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, event.host(), "Sinkhole routing or DNS evidence was observed.");
    ...
    public void inspectFlow(FlowRecord flow, DetectionContext context) {
        ...
        boolean serviceMatch = service.contains("sinkhole") || flow.tags().containsKey("sinkhole") || flow.protocol().contains("sinkhole");
        boolean volumeMatch = flow.bytes() > 1000000 && "dns".equals("bytes");
        boolean scanMatch = flow.isLikelyScan() && ("dns".equals("port") || "sinkhole".equals("scan"));
        if (serviceMatch || volumeMatch || scanMatch) {
            flowMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, flow.sourceIp(), "Sinkhole routing or DNS evidence was observed.");
    ...
    public void inspectIndicator(ThreatIndicator indicator, DetectionContext context) {
        boolean labelMatch = indicator.labels().contains("sinkhole") || indicator.type().contains("sinkhole") || indicator.value().contains("sinkhole");
        if (labelMatch) {
            indicatorMatches++;
            Finding finding = new Finding(name(), Severity.MEDIUM, indicator.id(), "Sinkhole routing or DNS evidence was observed.");
