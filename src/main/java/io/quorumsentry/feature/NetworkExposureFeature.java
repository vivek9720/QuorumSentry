...
public final class NetworkExposureFeature implements TelemetryFeatureExtractor {
    ...
    public FeatureVector event(EventRecord event) {
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "62", 509));
        vector.put("containsToken", body.toLowerCase().contains("category") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "62", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("containsToken", indicator.value().toLowerCase().contains("type") ? 1 : 0);
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "62", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }

    private double selectFlowMetric(FlowRecord flow) {
        return switch ("destinationPort") {
