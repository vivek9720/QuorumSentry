...
public final class ContainerRuntimeFeature implements TelemetryFeatureExtractor {
    private final EntropyMeter entropy = new EntropyMeter();
    private final PortCatalog ports = new PortCatalog();

    @Override
    public String name() {
        return "ContainerRuntimeFeature";
    }

    @Override
    public FeatureVector event(EventRecord event) {
        FeatureVector vector = new FeatureVector();
        String primary = event.attribute("container");
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "85", 509));
        vector.put("containsToken", body.toLowerCase().contains("container") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "85", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "85", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
