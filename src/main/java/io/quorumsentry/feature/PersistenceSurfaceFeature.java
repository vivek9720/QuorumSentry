...
public final class PersistenceSurfaceFeature implements TelemetryFeatureExtractor {
    private final EntropyMeter entropy = new EntropyMeter();
    private final PortCatalog ports = new PortCatalog();

    @Override
    public String name() {
        return "PersistenceSurfaceFeature";
    }

    @Override
    public FeatureVector event(EventRecord event) {
        FeatureVector vector = new FeatureVector();
        String primary = event.attribute("registry");
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "71", 509));
        vector.put("containsToken", body.toLowerCase().contains("registry") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "71", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "71", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
