...
public final class GeoVarianceFeature implements TelemetryFeatureExtractor {
    ...
    public FeatureVector event(EventRecord event) {
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "70", 509));
        vector.put("containsToken", body.toLowerCase().contains("src_ip") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "70", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("containsToken", indicator.value().toLowerCase().contains("validUntil") ? 1 : 0);
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "70", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }

    private double selectFlowMetric(FlowRecord flow) {
        return switch ("sourceIp") {
