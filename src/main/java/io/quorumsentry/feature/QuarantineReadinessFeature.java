...
public final class QuarantineReadinessFeature implements TelemetryFeatureExtractor {
    private final EntropyMeter entropy = new EntropyMeter();
    private final PortCatalog ports = new PortCatalog();

    @Override
    public String name() {
        return "QuarantineReadinessFeature";
    }

    @Override
    public FeatureVector event(EventRecord event) {
        FeatureVector vector = new FeatureVector();
        String primary = event.attribute("quarantine");
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "99", 509));
        vector.put("containsToken", body.toLowerCase().contains("quarantine") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "99", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "99", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }

    private double selectFlowMetric(FlowRecord flow) {
        return switch ("privileged") {
