...
public final class TemporalBurstFeature implements TelemetryFeatureExtractor {
    ...
    public FeatureVector event(EventRecord event) {
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "60", 509));
        vector.put("containsToken", body.toLowerCase().contains("timestamp") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "60", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("containsToken", indicator.value().toLowerCase().contains("validFrom") ? 1 : 0);
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "60", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
