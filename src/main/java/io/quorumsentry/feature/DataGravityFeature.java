...
public final class DataGravityFeature implements TelemetryFeatureExtractor {
    ...
    public FeatureVector event(EventRecord event) {
        ...
        vector.put("saltedHostBucket", TokenNormalizer.stableBucket(event.host() + "67", 509));
        vector.put("containsToken", body.toLowerCase().contains("dataset") ? 1 : 0);
    ...
    public FeatureVector flow(FlowRecord flow) {
        ...
        vector.put("saltedPairBucket", TokenNormalizer.stableBucket(flow.sourceIp() + flow.destinationIp() + "67", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
    ...
    public FeatureVector indicator(ThreatIndicator indicator) {
        ...
        vector.put("containsToken", indicator.value().toLowerCase().contains("severity") ? 1 : 0);
        vector.put("saltedIndicatorBucket", TokenNormalizer.stableBucket(indicator.id() + "67", 4093));
        vector.put("normalizedMagnitude", vector.magnitude());
        return vector;
    }
